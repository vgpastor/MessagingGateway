...
// Set env for schema export
process.env['NODE_ENV'] = 'development';
process.env['SWAGGER_ENABLED'] = 'true';
process.env['API_KEY'] = 'schema-export';

const { loadAccountsFromYaml } = await import('../src/infrastructure/config/accounts.loader.js');
...
const webhookConfigRepo = await FileWebhookConfigStore.create(resolve(process.cwd(), '.tmp-webhooks.json'));
...
try { unlinkSync(resolve(process.cwd(), '.tmp-webhooks.json')); } catch {}
